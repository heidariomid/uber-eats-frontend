					...
					<ul className='mt-6  grid grid-cols-1 gap-5 sm:gap-6 sm:grid-cols-2 lg:grid-cols-3 '>
						{items?.map((dish, i) => (
							<li key={dish.id} className='col-span-1 flex shadow-sm rounded-md  '>
								<div className=' flex-1 flex items-center justify-between shadow-md border-t-2 rounded-r-md truncate pr-10'>
												...
												{options.map(
													(dishOption) =>
														dishOption.dishId === dish.id && (
															<div key={dishOption.id} className='text-gray-600 flex justify-around  '>
