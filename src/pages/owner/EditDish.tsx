...
const EditDish = () => {
	...
	const onValidSubmit = async () => {
		...
		if (data?.getDish?.ok && !loadingDish && !error && !loading && isValid) {
			dispatch({variables: {data: {dishId: Number(dishId), options: optionItem, name: name && name !== dish.name ? name : undefined, description: description && description !== dish.description ? description : undefined, price: price && Number(price) !== Number(dish.price) ? Number(price) : undefined, photo: photoUrl && photoUrl !== dish.photo ? photoUrl : undefined}}});
		}
	};
						...
						<form className='flex flex-col w-full mt-5 px-10 bg-white ' onSubmit={handleSubmit(onValidSubmit)}>
							...
							{newOption &&
								...
								newOption?.map((option, index) => {
									return (
										<div key={`options[${index}]`} className=' w-full '>
											{/* @ts-ignore */}
